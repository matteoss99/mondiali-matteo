...
const CATEGORY_MAP: { [key: string]: string } = {
  'Italy': 'Italia', 'England': 'Inghilterra', 'Spain': 'Spagna',
  'Germany': 'Germania', 'France': 'Francia', 'Europe': 'Europa',
  'International': 'Europa', 'World': 'Mondo'
};
...
export default function PronosticiPage() {
  ...
  // --- STATI PER SALVARE I PRONOSTICI ---
  ...
  const fetchScheduledMatches = async () => {
    ...
    try {
      ...
      const sortedCats = Array.from(foundCategories).sort((a, b) => PRIORITY.indexOf(a) - PRIORITY.indexOf(b));
      setAllMatches(list);
  ...
  useEffect(() => {
    if (selectedCategory === 'Tutte') setFilteredMatches(allMatches);
    else setFilteredMatches(allMatches.filter(m => m.category === selectedCategory));
  }, [selectedCategory, allMatches]);

  // --- FUNZIONI MOTORE: Gestione Numeri e Salvataggio ---
  ...
  const handleSavePrediction = async (match: any) => {
    ...
    // Controlla se ha inserito entrambi i numeri
    if (!matchScores || matchScores.home === '' || matchScores.away === '') {
      alert('⚠️ Devi inserire entrambi i risultati prima di salvare!');
      return;
    }
    ...
    try {
      ...
      if (!user) {
        alert('❌ Devi fare il login per poter salvare un pronostico.');
        setSavingId(null);
        return;
      }
      ...
      const { error } = await supabase.from('predictions').upsert({
        user_id: user.id,
        match_id: match.id.toString(), // Assicura che sia una stringa
      ...
      alert(`✅ Pronostico salvato con successo: ${match.homeTeam} ${matchScores.home} - ${matchScores.away} ${match.awayTeam}`);
    } catch (err) {
        ...
        <div className="flex gap-4 overflow-x-auto pb-6 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {categories.map((cat) => (
            <button
              ...
              className={`px-10 py-4 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] transition-all border-2 whitespace-nowrap
                ${selectedCategory === cat ? 'bg-blue-600 border-blue-400 text-white shadow-[0_0_30px_rgba(37,99,235,0.3)] scale-105' : 'bg-slate-900 border-slate-800 text-slate-500 hover:border-slate-600'}`}
            >
              {cat}
            </button>
          ))}
        ...
        {filteredMatches.length === 0 ? (
          ...
          filteredMatches.map((m) => (
            <div key={m.id} className="bg-slate-900/40 backdrop-blur-xl border border-slate-800/50 p-10 rounded-[3.5rem] shadow-2xl flex flex-col group hover:border-blue-600/50 transition-all">
              ...
              <div className="flex items-center justify-between gap-6 mb-10">
                ...
                <div className="flex gap-3">
                  <input 
                    ...
                    placeholder="0" 
                    className="w-14 h-14 bg-slate-950 border-2 border-slate-800 rounded-2xl text-center font-black text-xl text-blue-500 outline-none focus:border-blue-600 transition-all" 
                  />
                  <input 
                    type="number" 
              ...
              <button 
                onClick={() => handleSavePrediction(m)}
                disabled={savingId === m.id}
                className="w-full py-5 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 rounded-2xl font-black uppercase text-[11px] tracking-[0.3em] text-white transition-all shadow-xl active:scale-95"
              >
                {savingId === m.id ? 'Salvataggio...' : 'Salva Pronostico'}
              </button>
            </div>
          ))
        )}
